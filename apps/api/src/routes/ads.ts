...
adsRouter.get('/accounts', async (req, res) => {
  // #region agent log
  fetch('http://127.0.0.1:7290/ingest/b2c4cb70-d50c-4374-bc86-0b8f1906e582',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'1b185d'},body:JSON.stringify({sessionId:'1b185d',location:'ads.ts:GET /accounts:entry',message:'GET /accounts hit',data:{user:req.user ?? null},timestamp:Date.now(),hypothesisId:'CORS-H3'})}).catch(()=>{});
  // #endregion
  try {
    const accounts = await getAccountsList();
    // #region agent log
    fetch('http://127.0.0.1:7290/ingest/b2c4cb70-d50c-4374-bc86-0b8f1906e582',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'1b185d'},body:JSON.stringify({sessionId:'1b185d',location:'ads.ts:GET /accounts:ok',message:'GET /accounts success',data:{count:accounts.length},timestamp:Date.now(),hypothesisId:'CORS-H3'})}).catch(()=>{});
    // #endregion
    return res.json(accounts);
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Unknown error';
    // #region agent log
    fetch('http://127.0.0.1:7290/ingest/b2c4cb70-d50c-4374-bc86-0b8f1906e582',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'1b185d'},body:JSON.stringify({sessionId:'1b185d',location:'ads.ts:GET /accounts:error',message:'GET /accounts error',data:{message},timestamp:Date.now(),hypothesisId:'CORS-H4'})}).catch(()=>{});
    // #endregion
    return res.status(500).json({ error: message });
  }
});
...
adsRouter.get('/manual/:adId', async (req, res) => {
  try {
    ...
    if (!row) return res.status(404).json({ error: 'Anuncio no encontrado' });
    return res.json(row);
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Unknown error';
    return res.status(500).json({ error: message });
