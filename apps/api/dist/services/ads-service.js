 ...
 */
function countCalificadosByAdId(leads) {
    const map = new Map();
 ...
 */
function extractAudienceFromAdName(adName) {
    if (!adName || typeof adName !== 'string')
        return '';
    const lower = adName.toLowerCase();
    if (lower.includes('ddo'))
        return 'DDO';
 ...
 */
function computePuntaje(tasaCali, ctr) {
    if (tasaCali === 0)
        return Math.min(30, Math.round(ctr * 50));
    if (tasaCali >= 100)
        return 70;
    if (tasaCali >= 60)
        return 50;
...
export async function getAdsReport(params) {
